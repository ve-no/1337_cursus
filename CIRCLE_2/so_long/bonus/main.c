...
/*   Updated: 2023/01/17 11:54:55 by ael-bako         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_mlx_string_put(t_program *ap, int mv)
{
	...
	// if (ft_strchr("0", ap->data->map[ap->data->y][ap->data->x]))
	// 	ap->render = 1;
...
int	move_enemy(t_program *ap)
{
	...
	while (ap->data->map[++y])
	{
		...
		while (ap->data->map[y][++x])
		{
			if (ap->data->map[y][x] == 'Z' && ap->data->map[y][x + 1] == '0')
			{
				ap->data->map[y][x + 1] = 'Z';
				ap->data->map[y][x] = '0';
			}
...
int	main(int ac, char **av)
{
	...
	// mlx_loop_hook(ap->mlx, move_enemy, ap);
	mlx_loop(ap->mlx);
	return (0);
}
