...
/*   Updated: 2023/06/23 18:50:39 by ael-bako         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static int	ft_count_w(const char *s, char *c, int i[2])
{
	int		q[2];
...
static char	**ft_fill_tab(char **aux, char const *s, char *set, int i[3])
{
	int		s_len;
	int		q[2];
...
char	**ft_cmdtrim(char const *s, char *set)
{
	...
	if (!s)
		return (NULL);
	nwords = ft_count_w(s, set, counts);
	if (nwords == -1)
		return (NULL);
	aux = malloc((nwords + 1) * sizeof(char *));
	if (aux == NULL)
		return (NULL);
	aux = ft_fill_tab(aux, s, set, i);
	aux[nwords] = NULL;
	return (aux);
}
/* ft_strtrim_all */
